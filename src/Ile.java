...
/**
 * Classe Ile
 * Cette classe permet la cr�ation d'une �le
 * @author Team J3
 *
 */
public class Ile {
	...
	/**
	 * Fonction de r�cup�ration de la position d'une entit� voulu sur la carte( ATTENTION ! ne pas mettre de type susceptible d'�tre trouv� plusieurs fois)
	 * @return Position
	 */
	...
	public void deplacer(Personnage perso, int x){
		if(x == 1){
			...
			} else if(estNavire(pos,perso.getEquipe())){ // Verifie si c'est le navire alli�
				if(perso.getSurnavire()){ // Est sur navire
					plateau[perso.getPos().x][perso.getPos().y].setType(2);
					perso.setSurnavire(false);
				}else{ // Sur sol
